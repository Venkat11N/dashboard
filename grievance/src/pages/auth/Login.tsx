...
export default function Login() {
  ...
  async function handleLogin(e: React.FormEvent) {
    ...
    try {
      // 1. Verify Password First (This stays the same)
      ...
      const freeOtp = Math.floor(100000 + Math.random() * 900000).toString();
      ...
      const { error: dbError } = await supabase
        ...
        .upsert({ 
          email: email.trim(), 
          otp_code: freeOtp,
          expires_at: new Date(Date.now() + 10 * 60000).toISOString() // 10 min expiry
        });
      ...
      console.log("FREE SECURITY CODE FOR DEVELOPMENT:", freeOtp);
